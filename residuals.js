...
d3.select("#canvas_bubbles").remove();

let canvas_bubbles = d3.select("body").append("svg").attr("id","canvas_bubbles");
...
let w=+d3.select("#canvas_bubbles").style("width").slice(0,-2);

let h=+d3.select("#canvas_bubbles").style("height").slice(0,-2); 
...
let mouseover = (d,i)=>{
toolTip.style("visibility","visible").html("Country: "+i[0]+"<br>" + "Population: "+i[2]+"<br>"+"Happiness Score: "+i[3]+"<br>"+dim+": " +i[4]+"%").style("background-color","#ffffe0").style("color","darkblue").style("border","1px solid darkblue").style("left",0.5*(w_s-w-30)+0.4*w+"px").style("top",Math.min(yAxisScale(3.6),240)+"px").style("font",`${0.5*(h/42+w/42)}px arial`);}

let pad_t=Math.floor(h/10);
let pad=Math.floor(w/12);
let xScale=d3.scaleLinear().domain(scales[dim]).range([pad,w-pad]);
let yScale=d3.scaleLinear().domain([0,10]).range([0,h-2*pad_t]);
let yAxisScale=d3.scaleLinear().domain([0,10]).range([h-pad_t,pad_t]); 
let xAxis=d3.axisBottom(xScale).ticks(4);
let yAxis=d3.axisLeft(yAxisScale);

canvas_bubbles.append('g').call(xAxis).attr('id','x-axis').attr('transform','translate(0,'+(h-pad_t)+')');

canvas_bubbles.append('g').style("font","10px arial").call(yAxis).attr('id','y-axis').attr('transform','translate('+pad+',0)');
...
canvas_bubbles.selectAll('circle').data(dat).enter().append('circle').attr('cx',(item)=>{return xScale(item[4])}).attr('cy',(item)=>{ return  h-pad_t-yScale(item[3])}).attr('r',(item)=>(0.0000025*h)*Math.sqrt(item[2])).attr("fill",(item)=>item[5]).attr("opacity",0.7).on("mouseover",mouseover).on("mouseleave",()=>{return toolTip.style("visibility","hidden")});

legbox.append("text").text("Percentage related to sum of all dimensions").style("text-anchor","end").attr("x",xScale(scales[dim][0])+0.82*w).attr("y",yAxisScale(1.58)+8*(leg_h/7)+5).style("font",`${0.5*(w/40+h/40)}px arial`).style("fill","navy");
}
