...
let mouseover = (d,i)=>{
    ...
    toolTip.style("visibility","visible").html("Country: "+i[0]+"<br>" + "Population: "+i[2]+"<br>"+"Happiness Score: "+i[3]+"<br>"+dim+": " +i[4]+"%").style("background-color","#ffffe0").style("color","darkblue").style("border","1px solid darkblue").style("left",0.5*(w_s-w)+0.63*w+"px").style("top",Math.min(250,0.8*h_s)+"px").style("font",`${h/35}px arial`);}
...
legbox.append("text").text(reg2[i]).style("text-anchor","end").attr("x",xScale(scales[dim][0])+0.43*w).attr("y",yAxisScale(1.58)+i*(leg_h/6)).style("font",`${Math.min(w/36,h/36)}px arial`).style("fill","navy");}
...
legbox.append("text").text(reg2[i]).style("text-anchor","end").attr("x",xScale(scales[dim][0])+0.82*w).attr("y",yAxisScale(1.58)+(i-5)*(leg_h/6)).style("font",`${Math.min(w/36,h/36)}px arial`).style("fill","navy");}
...
legbox.append("text").text("Percentage related to sum of all dimensions").style("text-anchor","end").attr("x",xScale(scales[dim][0])+0.82*w).attr("y",yAxisScale(1.58)+8*(leg_h/7)).style("font",`${Math.min(8,h/30)}px arial`).style("fill","navy");
