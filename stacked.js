...
let canvas_c9 = d3.select("body")

    .append("svg")

    .attr("id","canvas_c9");

let legbox=canvas_c9.append('g').attr("id","legend");

let leg_h=+d3.select("#canvas_c9").style("height").slice(0,-2); 

let leg_w=+d3.select("#canvas_c9").style("width").slice(0,-2); 
...
canvas_c8.append("text").text("2022").style("text-anchor","end").attr("x",35*w/365).attr("y",29*h/40).style("font",`${14*w/365}px arial`).style("fill","navy");
...
canvas_c8.selectAll('rect').data(dim22[cnt]).enter().append('rect').attr('width', (item)=>{return item[0]*0.1*w}).

attr('height',0.8*h).attr('x',(item)=>{return 0.125*w+item[1]*0.1*w}).attr('y',10).attr("fill",(item)=>item[2]).on("mouseover",mouseover8).on("mouseleave",()=>{return toolTip8.style("visibility","hidden")});
...
canvas_c8.append("text").text(l22[0].toFixed(3)).style("text-anchor","end").attr("x",l22[0]*0.1*w+0.21*w).attr("y",27*h/40).style("font",`${11*w/365}px arial`).style("fill","navy");
