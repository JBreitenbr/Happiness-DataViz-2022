...
let pad_b=Math.floor(h/3);
let pad=Math.floor(w/12);
  
let mouseover = (d,i)=>{
console.log(cnt_lst);
console.log(cnt_lst[cnt_lst.length-1]);
toolTip.style("visibility","visible").html(i[0]+"<br>" + i[1]).style("background-color","#ffffe0").style("color","darkblue").style("border","1px solid darkblue").style("width",0.18*w+"px").style("text-align","center").style("display","block").style("top",0.65*h+"px").style("left",i[0]==cnt_lst[cnt_lst.length-2]?0.45*(w_s-w)+xScale(i[0])-25+"px":(i[0]==cnt_lst[cnt_lst.length-1]?0.45*(w_s-w)+xScale(i[0])-20+"px":0.45*(w_s-w)+xScale(i[0])+"px")).style("font",`${w/40}px arial`);}
...
canvas_c.append('g').call(xAxis).attr('transform','translate(0,'+(h-pad_b)+')').selectAll("text")  
    ...
    .attr("dx", "-.8em")
    .attr("dy", ".15em")
    .attr("transform", "rotate(-65)").style("font",`${Math.min(h/32,w/32)}px arial`);}
if((reg=="North America and ANZ")||(reg=="East Asia")||(reg=="South Asia")||(reg=="Southeast Asia")){
canvas_c.append('g').call(xAxis).attr('transform','translate(0,'+(h-pad_b)+')').selectAll("text")  
    .style("text-anchor", "end")
    .attr("dx", "-.8em")
    .attr("dy", ".15em")
    .attr("transform", "rotate(-65)").style("font",`${Math.min(h/24,w/24)}px arial`);}
  if(reg=="Sub-Saharan Africa"){
canvas_c.append('g').call(xAxis).attr('transform','translate(0,'+(h-pad_b)+')').selectAll("text")  
    .style("text-anchor", "end")
    .attr("dx", "-.8em")
    .attr("dy", ".15em")
    .attr("transform", "rotate(-65)").style("font",`${Math.min(h/36,w/35)}px arial`);}
  if(reg=="Commonwealth of Independent States"){
canvas_c.append('g').call(xAxis).attr('transform','translate(0,'+(h-pad_b)+')').selectAll("text")  
    .style("text-anchor", "end")
    .attr("dx", "-.8em")
    .attr("dy", ".15em")
    .attr("transform", "rotate(-65)").style("font",`${Math.min(h/28,w/28)}px arial`);}
canvas_c.append('g').style("font",`${w/35}px times`).call(yAxis).attr('id','y-axis').attr('transform','translate('+pad+','+0*pad+')');
canvas_c.selectAll('rect').data(vals22[reg]).enter().append('rect').attr('class','bar').attr('width', (w-2*pad)/(cnt_lst.length)).
